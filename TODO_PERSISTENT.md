...
## [x] ✂️ Extract Figure / Snip from PDF — region-crop tool — COMPLET DONE 2026-06-04 (Basic + Advanced high-DPI/batch/OCR + "Extra" transparent/trim/TIFF/SVG — toate LIVE)
...
4. **[x] Extra** — DONE 2026-06-04 (LIVE, commit `af0f616`). Familia extract-region (web + `/api/v1`) extinsă cu opțiuni de output: `fmt=png|tiff`, `transparent` (render alpha), `trim` (auto-crop bordură uniformă) + endpoint nou `/api/extract-region-svg` + `/api/v1/extract-region-svg` (SVG vectorial real — fitz page-SVG cu viewBox croit pe selecție). Cores partajate (`_region_fmt`, `_region_image_bytes` via Pillow, `_do_extract_region*` + `_do_extract_region_svg`); calea default png/no-trim/opaque păstrează fast-path-ul `pix.tobytes` byte-identic. UI: picker format (PNG/TIFF/SVG) + toggle transparent + auto-trim + buton SVG (DPI ascuns la vector); batch face fallback PNG la SVG. `/api-docs` documentează params + endpoint SVG. **Verificat live pe gold (revertat la free)**: tiff→image/tiff, transparent→PNG mode RGBA, trim→922×932<1117×1124, svg→`<svg>` vector, default→PNG RGB neschimbat. split-ocr + vecini 200.
...
- **Phase 2b — DONE 2026-06-04 (LIVE, commit `c42b192`)**: 7 endpoint-uri noi key-authed sub `/api/v1`: `pdf-to-word`, `word-to-pdf`, `pdf-to-excel`, `excel-to-pdf`, `pdf-to-powerpoint`, `powerpoint-to-pdf`, `ocr-pdf` (text-layer searchable). **Fără LibreOffice** — cores pure in-memory (`_v1_*_bytes`) cu aceleași motoare ca web-app-ul (fitz + python-docx/openpyxl/python-pptx/Pillow/RapidOCR), sincron + binary direct; **zero modificări pe rutele web** (același algoritm, duplicare intenționată low-drift — un viitor session poate unifica dacă apare drift). Guards: size-cap per-plan (`_v1_check_size`), 200-page cap pe conversiile PDF-input (`_guard_convert_pages`), 100-page cap OCR (`MAX_OCR_PAGES`), 400 pe fișier office invalid (`_open_office`). `/api-docs` listează toate 7 + cap-urile. **Verificat live pe gold (revertat garantat la free)**: cele 7 → 200 cu magic corect (504b office / %PDF), ocr-pdf produce PDF searchable (text extras = input), input greșit → 400, fără-cheie → 401. split-ocr (NO-TOUCH) + vecini L41 toți 200.
